...
public class EntityMaskCommand {
    public static void init() {
        ClientCommandRegistrationCallback.EVENT.register((event, access) -> {
            event.register(ClientCommandManager.literal("fireblanket:entitymask")
                    .then(
                            ClientCommandManager.literal("add")
                                    .then(ClientCommandManager.argument("type", RegistryEntryArgumentType.registryEntry(access, RegistryKeys.ENTITY_TYPE))
                                            .executes(client -> {
